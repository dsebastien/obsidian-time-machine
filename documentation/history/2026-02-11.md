...
- Added `options.intervalMinutes` to `FileRecoveryPlugin` type in `obsidian-internals.d.ts`
- Added `FileRecoveryService.getSnapshotIntervalMs()`: reads the file-recovery interval config, falls back to 5 minutes
- Registered `window.setInterval` via `this.registerInterval()` in plugin at the file-recovery interval
- On each tick, checks if any Time Machine views are open with a current file; if so, calls `updateForFile()` (full IndexedDB re-fetch + filter + re-render)
- No-op when no views are open — the interval fires but the callback exits immediately
- All validation passes: build, tsc, 53 tests, lint, format

## Git Integration (Snapshot Sources)

- Added unified `Snapshot` abstraction (`src/app/types/snapshot.intf.ts`): `SnapshotSource`, `Snapshot`, `FileRecoveryMetadata`, `GitMetadata`
- Created `src/app/domain/snapshot.ts`: factory functions (`fileRecoveryToSnapshot`, `gitCommitToSnapshot`), `mergeSnapshots`, `sortSnapshotsByDate`, `formatSnapshotLabel`
- Created `GitService` (`src/app/services/git.service.ts`): desktop-only git operations via dynamic `require('child_process').execFile`. Methods: `isAvailable`, `getVaultBasePath`, `getGitRelativePath`, `isFileTracked`, `getCommitsForFile`, `getFileAtCommit`. 10s timeout, 10MB buffer, graceful error handling.
- Created `SnapshotService` (`src/app/services/snapshot.service.ts`): orchestrator that fetches from both file-recovery and git sources, converts to unified `Snapshot[]`, merges chronologically
- Added `gitIntegrationEnabled` (default: true) and `gitMaxCommits` (default: 50) to `PluginSettings`
- Added "Git integration" settings section with toggle and slider (1-200) in settings tab
- Migrated `TimelineSliderComponent` from `FileRecoveryBackup` to `Snapshot` type, added source indicator (git-branch icon + commit info, or clock icon + "File recovery")
- Migrated `TimeMachineView` from `FileRecoveryService` to `SnapshotService`. Replaced `allBackups`/`backups` with `allSnapshots`/`snapshots`. View no longer blocks on file-recovery disabled if git snapshots could be available. Diff labels source-aware.
- Updated empty state hint: "Snapshots are created by File Recovery and git commits"
- Added source indicator CSS styles
- Added `Platform` and `FileSystemAdapter` to obsidian test mock
- Updated all test files to use `Snapshot` type and mock `SnapshotService`
- All validation passes: build, tsc, 100 tests, lint, format
