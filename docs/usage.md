...
Snapshots that are identical to the current file content are automatically hidden. Only snapshots that differ from your current file are shown. This means:

- The snapshot count in the header reflects only snapshots with actual differences
...
- **"No snapshots found for this file"** -- the file has no snapshots (or all snapshots are identical to the current content)
- **"File Recovery core plugin is not enabled"** -- enable it in **Settings -> Core plugins**
